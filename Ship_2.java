 ...
 */
public class Ship_2 extends Player {
     ...
     */
    public Ship_2 (int speed, int ycoor) {
        ...
        if (!died) {
            if (!getWorld().getObjects(Word.class).isEmpty()) {
                ...
                if (currTime > lastTime + timeloop && user_typed) {
                    lastTime = currTime;
                    lastTime = currTime;
                    Blast_2 blast = new Blast_2(bullet_speed, this.target_word);
                    ...
                    blast1.setVolume(30);
                    blast1.play();
                    getWorld().addObject(blast, getX() + 28, getY() - 11); // Modify the x and y coordinate arguments this later
                }
            }
        }
    }
